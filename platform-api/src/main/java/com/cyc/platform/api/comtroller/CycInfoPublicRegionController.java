...
public class CycInfoPublicRegionController {
	...
	public WebAsyncTask<String> findList(final CycInfoPublicRegion bean, HttpServletRequest request) {
		Callable callable = () -> {
			List<CycInfoPublicRegion> list = cycInfoPublicRegionService.findList(null);
			RetEntity retEntity = new ResponseListRet<CycInfoPublicRegion>(Tips.SUCCESS1.getCode(),
